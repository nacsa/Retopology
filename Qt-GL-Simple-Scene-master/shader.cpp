...
//      glProgramParameteri(handle, GL_GEOMETRY_VERTICES_OUT, 4);
//      glProgramParameteri(handle, GL_GEOMETRY_INPUT_TYPE, GL_POINTS);
//      glProgramParameteri(handle, GL_GEOMETRY_OUTPUT_TYPE, GL_TRIANGLE_STRIP);
...
void Shader::bindUniformMatrix3(const char* name, float* m, unsigned int arraySize){
  GLuint location = glGetUniformLocation(shaderProgram, name);
  glUniformMatrix3fv(location, arraySize, false, m);
}
...
void Shader::bindUniformMatrixMat4( const char *name, const mat4 & m)
{
...
void Shader::bindUniformMatrixMat3( const char *name, const mat3 & m)
{
    int loc = glGetUniformLocation(shaderProgram, name);
    if(loc >= 0)
    {
